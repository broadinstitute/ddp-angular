...
export class Search {
  ...
  public async dates(columnName: string, {from: fromValue, to: toValue, additionalFilters}: Partial<DateConfig>): Promise<void> {
    ...
    if(fromValue instanceof Date) fromDate = new Intl.DateTimeFormat('en-US').format(fromValue);
    else if(fromValue && this.enUSDateRegExp.test(fromValue)) fromDate = fromValue;
    else if(fromValue && this.todayTextInDateRegExp.test(fromValue)) await this.setTodayFor(columnName);
    else fromValue && console.warn(`'${fromValue}' - is not valid date, therefore can't be set in '${columnName}' date field`)

    if(toValue instanceof Date) fromDate = new Intl.DateTimeFormat('en-US').format(toValue);
    else if(toValue && this.enUSDateRegExp.test(toValue)) toDate = toValue;
    else if(toValue && this.todayTextInDateRegExp.test(toValue)) await this.setTodayFor(columnName, 1);
    else toValue && console.warn(`'${toValue}' - is not valid date, therefore can't be set in '${columnName}' date field`)
  ...
  private async setAdditionalFilters(columnName: string, additionalFilters: AdditionalFilter[] | null | undefined, isTextField: boolean = false): Promise<void> {
  ...
  private async setAdditionalFilter(columnName: string, additionalFilter: AdditionalFilter, isTextField: boolean = false): Promise<void> {
  ...
  private async setExactMatch(columnName: string, isTextField: boolean = false): Promise<void> {
  ...
  private async isAdditionalFiltersOpen(columnName: string, isTextField: boolean = false): Promise<boolean> {
    const baseColumnXPath = isTextField ? this.baseTextColumnXPath(columnName) : this.baseColumnXPath(columnName);
    return (await this.page.locator(baseColumnXPath + this.minusIconXPath)?.isVisible());
  }

  private async setTodayFor(columnName: string, nth: number = 0): Promise<void> {
  ...
  private checkboxLocator(columnName: string, checkboxName: string): Locator {
    return this.page
      .locator(`${this.baseColumnXPath(columnName)}//mat-checkbox[label[.//*[text()[normalize-space()='${checkboxName}']]]]`)
  }
  ...
  private additionalFilterCheckboxLocator(columnName: string, checkboxName: AdditionalFilter, isTextField: boolean = false): Locator {
    return this.page.locator(`${isTextField ? this.baseTextColumnXPath(columnName) : this.baseColumnXPath(columnName)}/div[mat-checkbox[label[*[text()[normalize-space()='${checkboxName}']]]]]/mat-checkbox`);
  }

  /* XPaths */

  private get openButtonXPath(): string {
    return `//div[text()[normalize-space()='Search'] and button[.//*[local-name()='svg' and @data-icon='search']/*[local-name()='path']]]/button`
  }
