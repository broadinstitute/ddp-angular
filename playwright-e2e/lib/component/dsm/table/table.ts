...
export class Table {
  ...
  public async openParticipantPageBy(columnName: string, columnValue: string, position: number = 1): Promise<ParticipantPage | undefined> {
    const proceed = !!columnName && !!columnValue && !!position;
    proceed && await this.page.locator(this.getParticipantXPathBy(columnName, columnValue)).nth(position - 1).click()
  ...
  public async selectParticipantAt(position: number): Promise<void> {
    return position ? await this.getParticipantAt(position).nth(0).locator('mat-checkbox').click() :
      undefined;
  }
  ...
  private getParticipantXPathBy(columnName: string, columnValue: string): string {
    return `//table/thead/th[text()[normalize-space()='${columnName}']]`
  ...
  private getParticipantDataXPathAtBy(position: number, columnName: string) {
    return `//table/tbody/tr[${position}]//td[position()=count(//table/thead/`
