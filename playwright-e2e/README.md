...
* If you don't set up `.env` file, you can also specify environment
  ...
  * For example, to run `login-visual.spec.ts` test:
  > npx cross-env SITE_PASSWORD=<SITE_PASSWORD> SINGULAR_USER_EMAIL=<EMAIL> singularUserPasswd=<YOUR_PASSWORD> SINGULAR_BASE_URL=<HOME_URL> npx playwright test --config=playwright.config.ts login-visual.spec.ts
