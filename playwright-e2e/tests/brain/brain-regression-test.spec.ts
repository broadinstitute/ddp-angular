...
test.skip('Brain statics @dss @brain', async ({ page }) => {
  ...
  await page
    .getByRole('heading', {
      name: 'We’re in the process of collecting data from participants across the United States and Canada.'
    })
    .click();
...
test.skip('Brain enroll kid on their behalf @dss @brain', async ({ page }) => {
  test.slow();
  await page.goto(BRAIN_BASE_URL!);
  ...
  await page
    .locator(
      'div:nth-child(9) > .ddp-single-question > div > ddp-activity-question > .ddp-activity-question > ddp-activity-answer > ddp-activity-text-answer > ddp-activity-text-input > .activity-text-input > .mat-form-field > .mat-form-field-wrapper > .mat-form-field-flex > .mat-form-field-infix'
    )
    .click();
...
test.skip('Brain enroll self @dss @brain', async ({ page }) => {
  test.slow();
  await page.goto(BRAIN_BASE_URL!);
  ...
  await page
    .getByRole('row', {
      name: 'Medical Release Form Thank you for providing your mailing address and contact information for your physician(s) and hospital(s)'
    })
  ...
  await email.checkUserReceivedEmails(userEmail, [
    {
      ...
      textProbe:
        'Thank you for joining the Brain Tumor Project. We are now asking if you would be willing to sign our research consent form, where we ask your permission to obtain copies of your medical records'
    },
    {
      subject: 'Thank you for providing your consent',
      textProbe: 'Thank you for joining The Brain Tumor Project and for giving us your consent for research'
    }
  ]);
});
