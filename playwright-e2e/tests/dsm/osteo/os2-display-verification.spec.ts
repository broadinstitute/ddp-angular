...
test.describe.serial(`OS2 Verify expected display of participant information`, () => {
  const adultPrequalifier = `I have been diagnosed with osteosarcoma and I'm signing myself up`;
  ...
  test(`OS2: Verify participants that can view Return of Results can be filtered for`, async ({ page, request }) => {
    ...
    await customizeViewPanel.selectColumns(CustomizeView.PREQUALIFIER_SURVEY, [Label.PREQUAL_SELF_DESCRIBE]);
    ...
    await searchPanel.checkboxes(Label.PREQUAL_SELF_DESCRIBE, { checkboxValues: [adultPrequalifier] });
    ...
    //await savedFilterSection.delete(savedFilterName); //delete late when adding test that need the saved filter
  })
