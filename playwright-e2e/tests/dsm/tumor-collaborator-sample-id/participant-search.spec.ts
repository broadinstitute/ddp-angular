...
test.describe.serial('Tumor Collaborator Sample ID', () => {
  ...
  const studies: StudyName[] = [StudyName.OSTEO2, StudyName.PANCAN, StudyName.BRAIN];
  ...
  for (const study of studies) {
    test(`Search by tumor sample id for non-legacy participant @dsm @${study}`, async ({ page, request }) => {
      ...
      const participantPage: ParticipantPage = await participantListTable.openParticipantPageAt({ position: row, cmiColumn: Label.SHORT_ID });
