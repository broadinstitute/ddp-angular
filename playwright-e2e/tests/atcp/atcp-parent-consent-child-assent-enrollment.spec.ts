...
/**
 * minor: under 18 years old in US
 */
test.describe('ATCP parent consent and child assent enrollment', () => {
  ...
  test('child is 13 years old @dss @atcp @visual', async ({ page }) => {
    ...
    await test.step("New Participant's Enrollment Process: Medical History", async () => {
      ...
      await medicalHistory.firstSymptomObservedAge.fill('10');
      await medicalHistory.neurologicProblemFirstSuspectedAge.fill('10');
