...
test('Static Content @osteo', async ({page}) => {
  ...
  await page.getByRole('heading', { name: 'Together, the osteosarcoma community has the power to move research forward' }).click();
  ...
  await page.locator('section').filter({ hasText: 'STEP 2 access_time5 minutes Tell us where you’ve been treated We ask where patie' }).getByRole('img', { name: 'Computer screen displaying medical center' }).click();
  await page.locator('section').filter({ hasText: 'STEP 3 access_timeTime varies by survey Respond to surveys about you and your ex' }).getByRole('img', { name: 'Computer screen displaying online survey' }).click();
  ...
  await page.getByRole('paragraph').filter({ hasText: '• Receive information about what we’ve learned from your tumor sample (optional)' }).click();
