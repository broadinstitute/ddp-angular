...
import { UserActivityServiceAgent, CompositeDisposable, ActivityInstance } from 'ddp-sdk';
...
@Component({
  selector: 'app-rarex-activity-redirect',
  template: ''
})
export class RarexActivityRedirectComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    let getActivities$: Observable<any> = this._userActivityServiceAgent
      .getActivities(
    ...
    if (instanceGuid) {
      getActivities$ = getActivities$.pipe(
        map(activities => {
          return activities ? activities.find(activity => activity.instanceGuid === instanceGuid) : null;
        })
      );
    }
    ...
    if (activityCode) {
      getActivities$ = getActivities$.pipe(
        map(activities => {
          return activities ? activities.find(activity => activity.activityCode === activityCode) : null;
        })
      );
    }
