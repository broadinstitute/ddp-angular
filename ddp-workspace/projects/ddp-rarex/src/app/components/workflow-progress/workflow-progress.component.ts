...
export class WorkflowProgressComponent implements OnChanges {
  ...
  private useLovedOneSurveySteps(aboutYouStatus = false): void {
    this.steps = [
      ...
      { number: 2, name: 'WorkflowProgress.LegacySurvey', isCompleted: false, activityCodes: ['LEGACY'] }
    ];
  }
  ...
  private useMainSurveySteps(aboutYouStatus = false): void {
    this.steps = [
      ...
      { number: 4, name: 'WorkflowProgress.DemographicsSurvey', isCompleted: false, activityCodes: ['DEMOGRAPHICS'] },
      { number: 5, name: 'WorkflowProgress.MedicalBackgroundSurvey', isCompleted: false, activityCodes: ['GENERAL_MEDICAL_BACKGROUND_SURVEY'] },
      { number: 6, name: 'WorkflowProgress.NeurodevelopmentSurvey', isCompleted: false, activityCodes: ['GENERAL_NEURODEVELOPMENT_SURVEY'] },
      { number: 7, name: 'WorkflowProgress.QualityOfLifeSurvey', isCompleted: false, activityCodes: ['GENERAL_QOL_SURVEY'] }
    ];
  }
}
