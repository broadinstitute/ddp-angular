...
export class ActivityTextInput implements OnInit, OnChanges, OnDestroy {
        ...
        suggestion.matches
            ...
            .forEach((match) => {
                ...
                taggedValue =
                    ...
                    openTag +
                    taggedValue.substring(
                        adjustedOffset,
                        adjustedOffset + match.length
                    ) +
                    closeTag +
                    taggedValue.substring(adjustedOffset + match.length);

                tagsAddedLength =
                    tagsAddedLength + openTag.length + closeTag.length;
            });
    ...
    private buildForm(): void {
        ...
        const formGroup = new FormGroup(
            ...
            {
                updateOn: this.block.hasUniqueValueValidator
                    ? 'blur'
                    : 'change',
            }
        );
        ...
        this.sub = this.formGroup.valueChanges
            .pipe(
                map((values) => ({
                        [this.controlName]: values[this.controlName]
                            ...
                            : null,
                        [this.confirmationControlName]: values[
                            this.confirmationControlName
                        ]
            ...
            .subscribe((values) => {
                ...
                this.block.setConfirmationAnswer(
                    values[this.confirmationControlName]
                );
