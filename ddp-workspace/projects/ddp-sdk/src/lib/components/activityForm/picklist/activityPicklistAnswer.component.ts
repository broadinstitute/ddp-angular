...
@Component({
    ...
    template: ` <ddp-question-prompt [block]="block"></ddp-question-prompt>
        <div>
            <ddp-activity-radiobuttons-picklist-question
                *ngIf="
                    block.selectMode === SELECT_MODE.SINGLE &&
                    block.renderMode === RENDER_MODE.LIST
                "
                [block]="block"
                [readonly]="readonly"
            ...
            <ddp-activity-checkboxes-picklist-question
                *ngIf="
                    (block.selectMode === SELECT_MODE.MULTIPLE &&
                        block.renderMode === RENDER_MODE.LIST) ||
                    block.renderMode === RENDER_MODE.CHECKBOX_LIST
                "
                [block]="block"
                [readonly]="readonly"
                (valueChanged)="valueChanged.emit($event)"
...
export class ActivityPicklistAnswer {
    ...
    @Output() valueChanged: EventEmitter<Array<ActivityPicklistAnswerDto>> =
        new EventEmitter();
