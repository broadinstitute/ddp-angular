...
docker build -f ddp-workspace/projects/ddp-dsm-ui/Dockerfile-base -t study-manager-base .

docker build -f Dockerfile-dsm -t study-manager-builder .

echo "Building angular app"
docker run -it -v "$(pwd)"/build:/build study-manager-builder

echo "Reading configs from ${PROJECT_ID} cloud secret manager"
gcloud --project=${PROJECT_ID} secrets versions access latest --secret="study-manager-ui-config" > ddp-workspace/projects/ddp-dsm-ui/src/assets/js/ddp_config.js
